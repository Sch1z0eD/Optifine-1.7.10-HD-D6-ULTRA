   ...
   public static TextureAnimation makeTextureAnimation(Properties props, ResourceLocation propLoc) {
      ...
      if(texFrom != null && texTo != null) {
         if(x >= 0 && y >= 0 && width >= 0 && height >= 0) {
            ...
            if(imageBytes == null) {
               Config.warn("patch.TextureAnimation: Source texture not found: " + texTo);
               return null;
            } else {
               ResourceLocation locTexTo = new ResourceLocation(texTo);
               if(!Config.hasResource(locTexTo)) {
                  Config.warn("patch.TextureAnimation: Target texture not found: " + texTo);
                  return null;
               } else {
                  ITextureObject destTex = TextureUtils.getTexture(locTexTo);
                  if(destTex == null) {
                     Config.warn("patch.TextureAnimation: Target texture not found: " + locTexTo);
                     return null;
                  } else {
            ...
            Config.warn("patch.TextureAnimation: Invalid coordinates");
            return null;
         }
      } else {
         Config.warn("patch.TextureAnimation: Source or target texture not specified");
         return null;
      }
   }
