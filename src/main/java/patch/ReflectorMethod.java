...
public class ReflectorMethod {
   ...
   public Class[] targetMethodParameterTypes;
         ...
         if(cls == null) {
            ...
            try {
               if(this.targetMethodParameterTypes == null) {
                  ...
                  if(e.length <= 0) {
                     Config.log("(patch.Reflector) Method not present: " + cls.getName() + "." + this.targetMethodName);
                     return null;
                  }

                  if(e.length > 1) {
                     Config.warn("(patch.Reflector) More than one method found: " + cls.getName() + "." + this.targetMethodName);

                     for(int i = 0; i < e.length; ++i) {
                        Method m = e[i];
                        Config.warn("(patch.Reflector)  - " + m);
                     }
