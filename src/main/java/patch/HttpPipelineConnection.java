...
public class HttpPipelineConnection {
   ...
   public void addRequest(HttpPipelineRequest pr, List list) {
      list.add(pr);
      this.notifyAll();
   }
   ...
   public HttpPipelineRequest getNextRequest(List list, boolean remove) throws InterruptedException {
      while(list.size() <= 0) {
         this.checkTimeout();
         this.wait(1000L);
      }
